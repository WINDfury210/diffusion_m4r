...
def cosine_beta_schedule(num_timesteps, s=0.008):
    ...
    t = torch.linspace(0, num_timesteps, num_timesteps + 1, dtype=torch.float32) / num_timesteps
...
def train(model, dataloader, diffusion, optimizer, scheduler, device, num_epochs=50):
    ...
    torch.save(model.state_dict(), "models/cond_mnist_mlp_cosine.pth")
    plt.plot(loss_history)
    plt.xlabel("Epoch")
    plt.ylabel("Average Loss")
    plt.savefig("images/loss_curve_cosine.png")
    plt.close()
    print("Model saved to models/cond_mnist_mlp_cosine.pth")
    return loss_history
    ...
    with torch.no_grad():
        for method_config in sampling_methods:
            ...
            try:
                ...
                os.makedirs("images", exist_ok=True)
                plt.savefig(f"images/generated_mnist_{method}_cosine.png")
                print(f"Samples saved to images/generated_mnist_{method}_cosine.png")
                plt.close(fig)
