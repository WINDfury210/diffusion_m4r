...
class UnconditionalUNet(nn.Module):
    def __init__(self, input_dim, output_dim, time_dim=512, channels=[32, 64, 128, 128, 256, 512]):
        super().__init__()
...
class UnconditionalDiffusionModel(nn.Module):
    def __init__(self, input_dim, output_dim, time_dim=512, channels=[32, 64, 128, 128, 256, 512]):
        super().__init__()
        self.unet = UnconditionalUNet(input_dim, output_dim, time_dim, channels)
...
@torch.no_grad()
def generate(model, diffusion, device, input_shape, num_samples=100, steps=1000, method="ddpm", eta=0.0, lambda_corrector=0.0, clamp_range=(-1, 1)):
    model.eval()
    x = torch.randn((num_samples, *input_shape), device=device, dtype=torch.float32)
    ...
    for i in tqdm(range(steps), desc=f"Generating ({method})"):
        ...
        if method == "ddim":
            ...
            if lambda_corrector > 0 and t_next > 0:
                ...
                pred_noise_next = model(x, t_next_tensor)
...
def train(model, dataloader, diffusion, optimizer, scheduler, device, num_epochs=250):
    ...
    torch.save(model.state_dict(), "models/uncond_mnist_mlp.pth")
    plt.plot(loss_history)
    plt.xlabel("Epoch")
    plt.ylabel("Average Loss")
    plt.savefig("images/loss_curve_uncond.png")
    plt.close()
    print("Model saved to models/uncond_mnist_mlp.pth")
    return loss_history
...
if __name__ == "__main__":
    ...
    model = UnconditionalDiffusionModel(
        input_dim=1,
        output_dim=1,
        time_dim=512,
    ...
    with torch.no_grad():
        for method_config in sampling_methods:
            ...
            try:
                images = generate(
                    model, diffusion, device, input_shape,
                    num_samples=num_samples, steps=1000, method=method,
                    eta=method_config["eta"], lambda_corrector=method_config["lambda_corrector"],
                    clamp_range=(-1, 1)
                )
                ...
                for i in range(10):
                    for j in range(10):
                        ...
                        axes[i, j].axis("off")
                plt.suptitle(f"Unconditional Sampling Method: {method.upper()}")
                plt.tight_layout()
                
                os.makedirs("images", exist_ok=True)
                plt.savefig(f"images/generated_mnist_uncond_{method}.png")
                print(f"Samples saved to images/generated_mnist_uncond_{method}.png")
                plt.close(fig)
